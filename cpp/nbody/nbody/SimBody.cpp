...
void SimBody::applyForceFrom(double dt, const SimBody& other) {
	...
	if (!areSameBody(other)) {
		...
		if (manager.getConfig().solver == "rk4") {
			solver_rk4(dt, other);
		} else if (manager.getConfig().solver == "newtonian") {
			solver_newtonian(dt, other);
		}
	}
}
...
void SimBody::updatePosAndVel(double dt) {
	...
	pos += deltaPos + 0.5 * dt * (vel + oldVel);

	if (manager.getConfig().enableDebugOutput)
		std::cout << id << " New Pos And Vel " << pos << " " << vel << std::endl;
}

void SimBody::solver_rk4(double dt, const SimBody& other) {
...
void SimBody::solver_newtonian(double dt, const SimBody& other) {

	Vec3 relPos = pos - other.pos;
	deltaVel += dt * gravitationalAcceleration(other.mass, relPos);
}
